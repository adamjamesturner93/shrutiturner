...
import { ProgrammeContentLoading, DelayedProgrammeLoading } from "./loading";
import { useState, useEffect, useRef, useCallback } from "react";
import { WeekNavigation } from "./week-navigation";
import { ProgrammeNavigationLink } from "./navigation-link";
...
export function ProgrammePortal({ id }: { id: string }) {
  const pathname = usePathname();
  const path = pathname.split("/").filter(Boolean).slice(3);
  const { data: shell, error } = useProgrammeData<Awaited<ReturnType<typeof getProgrammeShell>>>(
    `/api/me/programmes/${id}/shell`
  );
  const section = path[0] || "home";
  const selectedWeek = path[1];
  const { data: sectionData, error: sectionError } = useProgrammeData<
    Awaited<ReturnType<typeof getProgrammeSection>>
  >(`/api/me/programmes/${id}/sections/${section}`);
  const p: Portal | null = shell
    ? {
        ...shell,
        introduction: "",
        equipment: "",
  ...
  const focusContent = useCallback(() => {
    if (!explicitNavigation.current) return;
    explicitNavigation.current = false;
    const target =
      contentRef.current?.querySelector<HTMLElement>("[data-week-heading], h2") ||
      contentRef.current;
    if (target) {
      target.tabIndex = -1;
  ...
  useEffect(() => {
    if (sectionData && !(section === "weeks" && selectedWeek)) focusContent();
  }, [pathname, sectionData, section, selectedWeek, focusContent]);
  ...
  return (
    <DashboardLayout handlesLegalAgreements title={p?.title || "Programme"}>
      ...
      >
        ...
        {p && (
          <header
            data-programme-header
            className="bg-brand-dark relative overflow-hidden rounded-[1.75rem] p-6 text-white md:p-9"
          >
            <div
              aria-hidden="true"
            ...
            <h1 className="relative max-w-3xl text-3xl md:text-4xl">{p.title}</h1>
            {p && (
        ...
        {!p && !error && <ProgrammeContentLoading header label="Loading your programme" />}
        {p && (
          <>
            {!p.accessible ? (
              ...
              <>
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <Button asChild variant="outline">
                    <Link scroll={false} href={`${base}/onboarding`}>
                ...
                >
                  {["Home", "Weeks", "Live", "Community", "Resources"].map((label) => (
                    <ProgrammeNavigationLink
                      key={label}
                      href={`${base}/${label.toLowerCase()}`}
                      active={section === label.toLowerCase()}
                      loading={section === label.toLowerCase() && !sectionData && !sectionError}
                    >
                      {label}
                    </ProgrammeNavigationLink>
                  ))}
                ...
                <div
                  ...
                  className="min-h-[32rem] scroll-mt-6 space-y-6"
                  aria-busy={!sectionData && !sectionError}
                >
                  {sectionError ? (
                    <p role="alert">{sectionError}</p>
                  ) : !sectionData ? (
                    <DelayedProgrammeLoading
                      key={section}
                      label={`Loading ${section === "live" ? "live sessions" : section}`}
                      kind={section}
                    />
                  ) : (
                    <>
                        ...
                        <>
                          ...
                          {!p.currentWeek && (
                            <article className={panelClass}>
                              ...
                              <p>
                                Your weekly education will appear here when the programme begins. In
                                the meantime, review your onboarding and add the live sessions to
                                your calendar.
                              </p>
                          ...
                          {!p.sessions.some((s) => s.upcoming) && (
                            <p className="text-muted-foreground">
                              No further live sessions are scheduled. Available recordings are in
                              Live.
                            </p>
                          )}
                          {p.sessions
                        ...
                        (path[1] ? (
                          <>
                            <WeekNavigation
                              ...
                              timezone={p.timezone}
                            />
                            <ProgrammeWeekView id={id} weekId={path[1]} onReady={focusContent} />
                            <WeekNavigation
                              onNavigate={() => {
                                explicitNavigation.current = true;
                              }}
                            ...
                            [...p.sessions]
                              ...
                              .map((s) => (
                                <article key={s.id} className={panelClass}>
                                  ...
                                  <When value={s.startsAt} />
                                  {s.canJoin && (
                                    <Button onClick={() => setJoining(s.id)}>
                                      Join live session
                                    </Button>
                                  )}
                                  {s.status === "completed" &&
                      ...
                      {p.credit?.status === "Eligible" && (
                        <article className={panelClass}>
                          ...
                          <p>
                            You have £{((p.credit.amountPence || 0) / 100).toFixed(2)} of your
                            programme fee available towards eligible 1:1 coaching if you join by{" "}
...
function ProgrammeWeekView({
  ...
  return (
    <section className="space-y-5">
      {error && <p role="alert">{error}</p>}
      {!w && !error && <DelayedProgrammeLoading key={weekId} label="Loading this week" />}
      {w && (
        <>
          <div className="border-brand-accent border-l-4 py-2 pl-5">
            <p className={eyebrow}>Week {w.number}</p>
            <h2 data-week-heading className="mt-2 scroll-mt-6 text-3xl">
              {w.title}
            </h2>
          </div>
          <article className={panelClass}>
