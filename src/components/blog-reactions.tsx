        ...
        <div className="flex items-center gap-3">
            <motion.div whileTap={isAuthenticated ? { scale: 1.3 } : undefined}>
                <Button
                    ...
                    className={`gap-2 rounded-full border px-3 py-2 transition-colors ${hasReacted
                        ? "border-[#d4183d]/20 bg-[#d4183d]/5 text-[#d4183d] hover:bg-[#d4183d]/10"
                        : "border-border text-muted-foreground hover:border-[#d4183d]/30 hover:text-[#d4183d]"
                        }`}
                    aria-label={hasReacted ? "Remove reaction" : "React with heart"}
                    aria-pressed={hasReacted}
                >
                    <Heart
                        className={`h-5 w-5 transition-all ${hasReacted ? "fill-[#d4183d] text-[#d4183d]" : ""
                            }`}
                    />
