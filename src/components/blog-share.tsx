...
interface BlogShareProps {
  /** Full URL of the post (will default to current window location) */
  url?: string;
  /** Article title for share text */
  title: string;
  /** Short summary for platforms that support it */
  excerpt?: string;
  /** Vertical or horizontal layout */
  variant?: "inline" | "sidebar";
}

export function BlogShare({ url, title, excerpt, variant = "inline" }: BlogShareProps) {
  ...
  return (
    <div
      className={
        isSidebar ? "flex flex-col items-center gap-2" : "flex flex-wrap items-center gap-2"
      }
    >
      {!isSidebar && <span className="text-muted-foreground mr-1 text-sm">Share:</span>}
      ...
      {typeof navigator !== "undefined" && "share" in navigator && (
        <Button variant="secondary" size="sm" className="rounded-full" onClick={handleNativeShare}>
          More...
        </Button>
      )}
    </div>
