-- Additive retreat checkout, room allocation and attendee claim support.

CREATE TYPE "RetreatInstalmentKind" AS ENUM (
  'deposit',
  'scheduled',
  'balance',
  'full_payment'
...
CREATE TYPE "RetreatInstalmentStatus" AS ENUM (
  'pending',
  'paid',
  'refunded',
  'cancelled',
...
CREATE TYPE "RetreatRoomUnitStatus" AS ENUM (
  'available',
  'assigned',
  'unavailable'
);

CREATE TYPE "RetreatAttendeeStatus" AS ENUM (
  'pending_claim',
  'claimed',
  'cancelled'
);
...
ALTER TABLE "RetreatDate"
  ...
  ADD COLUMN "payInFullDiscountEnabled" BOOLEAN NOT NULL DEFAULT false,
...
CREATE UNIQUE INDEX "RetreatRoomUnit_retreatDateId_label_key" ON "RetreatRoomUnit"("retreatDateId", "label");
CREATE INDEX "RetreatRoomUnit_roomOptionId_status_idx" ON "RetreatRoomUnit"("roomOptionId", "status");
CREATE INDEX "RetreatRoomUnit_retreatDateId_status_idx" ON "RetreatRoomUnit"("retreatDateId", "status");

CREATE INDEX "RetreatBooking_roomUnitId_bookingStatus_idx" ON "RetreatBooking"("roomUnitId", "bookingStatus");
...
CREATE UNIQUE INDEX "RetreatBookingInstalment_publicPaymentToken_key" ON "RetreatBookingInstalment"("publicPaymentToken");
CREATE UNIQUE INDEX "RetreatBookingInstalment_stripeCheckoutSessionId_key" ON "RetreatBookingInstalment"("stripeCheckoutSessionId");
CREATE UNIQUE INDEX "RetreatBookingInstalment_bookingId_sequence_key" ON "RetreatBookingInstalment"("bookingId", "sequence");
CREATE INDEX "RetreatBookingInstalment_bookingId_status_idx" ON "RetreatBookingInstalment"("bookingId", "status");
CREATE INDEX "RetreatBookingInstalment_status_dueAt_idx" ON "RetreatBookingInstalment"("status", "dueAt");
