...
describe("checkout-flow helpers", () => {
  it("builds membership checkout return paths with purchase context", () => {
    expect(buildMembershipCheckoutReturnPaths("annual")).toEqual({
      successPath:
        "/dashboard/coaching?checkout=success&purchase=retired-membership&interval=annual",
      cancelPath:
        "/dashboard/coaching?checkout=cancelled&purchase=retired-membership&interval=annual",
    });
  });

  it("builds credit checkout return paths with bundle context", () => {
    expect(buildCreditCheckoutReturnPaths(3)).toEqual({
      successPath: "/dashboard/coaching?checkout=success&purchase=retired-credits&bundle=3",
      cancelPath: "/dashboard/coaching?checkout=cancelled&purchase=retired-credits&bundle=3",
    });
  });

  it("preserves valid pricing checkout context in fallback paths", () => {
    expect(buildPricingFallbackPath({ kind: "credits", bundle: "10" })).toBe(
      "/dashboard/coaching?checkout=retry&checkoutError=1&purchase=retired-credits&bundle=10"
    );
    expect(buildPricingFallbackPath({ kind: "membership", interval: "annual" })).toBe(
      "/dashboard/coaching?checkout=retry&checkoutError=1&purchase=retired-membership&interval=annual"
    );
  });
